...
function App() {
  ...
  const [chatHistory, setChatHistory] = useState([{ text: "Hello Krishna! Main Riley AI hoon. Database active hai!", isBot: true }]);
  ...
  if (view === 'admin') return (
    <div className="min-h-screen bg-slate-50 p-8 font-sans">
      <header className="flex justify-between items-center mb-12 bg-white p-6 rounded-3xl shadow-sm border border-emerald-100">
        ...
        <button onClick={() => setView('home')} className="bg-red-50 text-red-600 px-6 py-2 rounded-full font-bold transition hover:bg-red-100">Exit Dashboard</button>
      </header>
      <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto text-center">
        <div className="bg-white p-10 rounded-[2.5rem] shadow-sm border border-emerald-50"><h3 className="text-gray-400 font-bold mb-2 uppercase text-xs">Total Orders</h3><p className="text-6xl font-black text-emerald-600">0</p></div>
        <div className="bg-white p-10 rounded-[2.5rem] shadow-sm border border-emerald-100"><h3 className="text-gray-400 font-bold mb-2 uppercase text-xs">Active AI Chats</h3><p className="text-6xl font-black text-emerald-600">1</p></div>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#fafaf9] font-sans">
      <nav className="fixed top-0 w-full z-[100] bg-white/90 backdrop-blur-lg border-b border-emerald-50 py-3 px-6 flex justify-between items-center shadow-sm">
        ...
        <div className="flex gap-4 items-center">
          <div className="hidden lg:flex gap-8 font-bold text-emerald-950 mr-4"><a href="#home">Home</a><a href="#about">Story</a><a href="#products">Collection</a></div>
          <button onClick={() => setIsLoggedIn(true)} className="text-emerald-700 bg-emerald-50 px-4 py-1.5 rounded-full text-xs font-bold border border-emerald-100 flex gap-1 items-center hover:bg-emerald-100 transition"><User size={14}/> {isLoggedIn ? 'Profile' : 'Sign In'}</button>
          <a href="#products" className="bg-emerald-600 text-white px-5 py-2 rounded-full font-bold text-sm shadow-lg no-underline hover:bg-emerald-700 transition">Shop Now</a>
        </div>
      </nav>

      <header id="home" className="relative h-screen flex items-center justify-center overflow-hidden">
        {/* Strictly Muted Video */}
      ...
      <section id="about" className="py-24 px-6 bg-gradient-to-br from-[#f0fff4] to-[#fff5f5]">
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-16 items-center text-center md:text-left">
          <div className="bg-white p-12 rounded-[3.5rem] shadow-2xl border border-emerald-50">
            ...
            <p className="text-gray-600 text-lg leading-relaxed">Directly collecting rice husk from farmers to prevent stubble burning.</p>
          </div>
      ...
      <section id="products" className="py-24 px-6 bg-white">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-8">
          {products.map(p => (
            <div key={p.id} className="bg-white p-6 rounded-[2.5rem] border border-emerald-50 shadow-sm text-center group hover:shadow-2xl hover:-translate-y-4 transition-all duration-500">
      ...
      {/* Riley Chat Window (Fixed Enter key) */}
      {chatOpen && (
        <div className="fixed bottom-24 right-10 w-80 bg-white rounded-[2rem] shadow-2xl border border-blue-50 z-[200] overflow-hidden animate-in fade-in slide-in-from-bottom-5">
          <div className="bg-blue-600 p-4 text-white flex justify-between items-center"><span className="font-bold flex gap-2 items-center"><MessageCircle size={18}/> Riley AI Support</span><button onClick={() => setChatOpen(false)}><X size={20}/></button></div>
          <div className="h-72 p-4 overflow-y-auto bg-slate-50 flex flex-col gap-3 text-sm">{chatHistory.map((chat, i) => (<div key={i} className={`p-3 rounded-2xl max-w-[85%] ${chat.isBot ? 'bg-blue-100 text-blue-950 self-start shadow-sm' : 'bg-blue-600 text-white self-end shadow-md'}`}>{chat.text}</div>))}<div ref={chatEndRef} /></div>
          <div className="p-4 border-t flex gap-2 bg-white"><input type="text" className="flex-1 outline-none text-sm" placeholder="Type a message..." value={message} onChange={(e) => setMessage(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSendMessage()} /><button onClick={handleSendMessage} className="text-blue-600"><Send size={20}/></button></div>
        </div>
      )}

      {/* SINGLE Riley AI Button (Double Button Fix) */}
      ...
      <footer className="bg-emerald-950 text-white py-20 px-10 text-center md:text-left">
        ...
        <p className="opacity-50 text-sm">© 2026 Prawali. Unit of Startly Innovations Pvt. Ltd. | Patna</p>
      </footer>
    </div>
  );
}
